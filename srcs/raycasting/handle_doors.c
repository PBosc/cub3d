...
/*   Updated: 2024/01/29 04:59:31 by pibosc           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static bool	detect_door(t_map *map, t_player *player, int coords[2])
{
	...
	if (valid_coordinates(map, map_x, map_y - 1)
		&& ft_in_charset("23", map->map[map_y - 1][map_x])
		&& 5 * PI / 4 < player->angle && player->angle <= 7 * PI / 4)
		return (set_coords(map_x, map_y - 1, coords), true);
	if (valid_coordinates(map, map_x, map_y + 1)
		&& ft_in_charset("23", map->map[map_y + 1][map_x])
		&& PI / 4 < player->angle && player->angle <= 3 * PI / 4)
		return (set_coords(map_x, map_y + 1, coords), true);
	if (valid_coordinates(map, map_x - 1, map_y)
		&& ft_in_charset("23", map->map[map_y][map_x - 1])
		&& ((7 * PI / 4 < player->angle && player->angle <= 2 * PI / 4)
		|| (0 <= player->angle && player->angle <= PI / 4)))
		return (set_coords(map_x - 1, map_y, coords), true);
	if (valid_coordinates(map, map_x + 1, map_y)
		&& ft_in_charset("23", map->map[map_y][map_x + 1])
		&& 3 * PI / 4 < player->angle && player->angle <= 5 * PI / 4)
		return (set_coords(map_x + 1, map_y, coords), true);
	return (false);
}
